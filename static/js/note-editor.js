  ...
  addInputRules() {
    ...
    if (this.options.keepMarks || this.options.keepAttributes) {
      inputRule = wrappingInputRule({
        ...
        keepMarks: this.options.keepMarks,
        keepAttributes: this.options.keepAttributes,
        getAttributes: (match2) => ({ start: +match2[1], ...this.editor.getAttributes(TextStyle2.name) }),
        joinPredicate: (match2, node) => node.childCount + node.attrs.start === +match2[1],
        editor: this.editor
      });
    }
...
var ListItem4 = Node3.create({
  name: "listItem"
});
var ListItem$1 = ListItem4.extend({
  /**
   * @return {{markdown: MarkdownNodeSpec}}
   */
  addStorage() {
...
var noteEditorTmpl = document.createElement("template");
...
`;
var NoteEditor = class extends HTMLElement {
